...
- Creare payload malevolo contenente istruzioni furtive.
...
- Configurare una semplice applicazione basata su  LLM che recuperi automaticamente i contenuti esterni (es. web scraping controllato, API simulate, file caricati dall’utente).
...
Per sostenere l'esame lo studente dovrà consegnare un file .zip contenente il database e un report che 1) per ogni programma vulnerabile spieghi che cosa fa il programma, dove e perche' e' presente la vulnerabilità e come può essere sfruttata dall'attaccante; per ogni programma non vulnerabile spieghi che cosa e perche' non e' vulnerabile alla CWE scelta. Il database dovrà essere organizzato in due sottocartelle, una contenente file vulnerabili alla CWE scelta e una cartella contenente i file non vulnerabili. Ad esempio: 
